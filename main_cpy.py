...
def validation_runs(*args):
    ...
    if len(args) == 0:
        display_prompt = "Choose for which data set you want to apply Backpropogation on MLP"
    ...
    #Breast Cancer Wisconsim data    
        ...
        file_name = "breatcancerwisconsin_output"+iterations+"_"+str(Eta)+"_.txt"
        f = open(file_name, "w")
...
#       simple binary classifier is built with single set of weights.
        ...
        trng, tst = split_data_trng_test( data_set )
        #Wh, Vi, Nh = AlgoFunc.calculate_weights_2class_backprop(data_set)
        #Wh, Vi, Nh = AlgoFunc.calculate_weights_2class_backprop(data_set,0.01,30)
           
        #No Layers
        Wh, Vi, layers, WIh = AlgoFunc.calc_wgts_backprop_multi_layers(trng,0.01,30,list(),1)
        print("No Layers\n")
        print("Accuracy = ", AlgoFunc.verify_convergence_MLP_backprop(tst, Wh, WIh, layers, False))

        #1 Layer        
        Wh, Vi, layers, WIh = AlgoFunc.calc_wgts_backprop_multi_layers(trng,0.01,30,[10],1)
        print("1 Layer\n")
        print("Accuracy = ", AlgoFunc.verify_convergence_MLP_backprop(tst, Vi, WIh, layers, False))

        #2 Layers
        Wh, Vi, layers, WIh = AlgoFunc.calc_wgts_backprop_multi_layers(trng,0.01,30,[10,5],1)
        print("2 Layers\n")
        print("Accuracy = ", AlgoFunc.verify_convergence_MLP_backprop(tst, Vi, WIh, layers, False))
...
#        #No Layers
#        Wh, Vi, layers, WIh = AlgoFunc.calc_wgts_backprop_multi_layers(trng,0.01,30,list())
#        print("No Layers\n")
#        print("Accuracy = ", AlgoFunc.verify_convergence_MLP_backprop(tst, Wh, WIh, layers))

#        #1 Layer        
#        Wh, Vi, layers, WIh = AlgoFunc.calc_wgts_backprop_multi_layers(trng,0.01,30,[10])
#        print("1 Layer\n")
#        print("Accuracy = ", AlgoFunc.verify_convergence_MLP_backprop(tst, Vi, WIh, layers))
#
        #2 Layers
        Wh, Vi, layers, WIh = AlgoFunc.calc_wgts_backprop_multi_layers(trng,0.01,50,[10,10])
        print("2 Layers\n")
        print("Accuracy = ", AlgoFunc.verify_convergence_MLP_backprop(tst, Vi, WIh, layers))        
